...
module.exports = class MalOauth {
	...
	access_token = async (req, res, next) => {
		...
		const rn = await fetch(`https://myanimelist.net/v1/oauth2/token`, {
			...
			body: `grant_type=authorization_code&code=${response.code}&client_id=${this[_clientID]}&client_secret=${this[_clientSecret]}&code_verifier=${this[_codeChallenge]}&redirect_uri=${this[_callback]}`,
		});
